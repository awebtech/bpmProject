...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter
{
	...
	*/
	function protect($password, $encoded = false)
	{
		$this->_protect   = 1;
		$this->_password  = ($encoded ? $password : $this->_encodePassword($password));
	}
	...
	*/
	...
	function writeRow($row, $col, $val, $format = null, $numberFormat = null)
	{
		$retval = '';
		if (is_array($val)) {
			foreach ($val as $v) {
				if (is_array($v)) {
					$this->writeCol($row, $col, $v, $format, $numberFormat = null);
				} else {
	...
	*/
	...
	function writeCol($row, $col, $val, $format = null, $numberFormat = null)
	{
		$retval = '';
		if (is_array($val)) {
			foreach ($val as $v) {
	...
	*/
	...
	var $_biff8_input_encoding = 'UTF-16LE';
	function setBIFF8InputEncoding($encoding) {
		...
		$this->_biff8_input_encoding = $encoding;
	}
